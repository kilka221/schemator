...
if (startIndex !== -1 && endIndex !== -1) {
    ...
    if (actualMaxY <= PAGE_H + 50) {
        ...
        for (let s = 0; s <= maxS; s++) {
            ...
            // contentMaxY is absolute. Let's make it local to page.
            let localContentMaxY = contentMaxY - yMin;
            ...
            allEdgesFinal.forEach(e => {
                ...
                if (eInS && newSegments.length > 0) {
                    ...
                    if (e.labelPos && e.labelPos.y >= yMin && e.labelPos.y < yMax) {
                         labelPos = { x: e.labelPos.x, y: e.labelPos.y - yMin };
                    }
                    // preserve Arrow unless noArrow is explicitly true
...
`;
    ...
    console.log("Improved Pagination injected V4.");
} else {
